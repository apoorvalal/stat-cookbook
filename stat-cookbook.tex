...
% ----------------------------------------------------------------------------
%
% Apoorva Lal, forked from Vallentin's code
...
% Personal
\def\email{lal.apoorva@gmail.com}
\def\web{---}
...
\graphicspath{{/home/alal/tmp/tex-sandbox/stat-cookbook}}
...
\begin{center}
  ...
  \footnotesize{
    ...
    \today\\[1ex]
    \web\\[1ex]
    \href{http://apoorvalal.github.io}{Apoorva Lal} \\
    Forked from original repo \url{http://statistics.zone/} by
...
\begin{multicols*}{3}
  ...
  \hrule
  \vspace{5pt}
  {\footnotesize This cookbook integrates various topics in
  probability theory and statistics, based on
  literature~\cite{Hoel72,Wasserman03,Shumway06} and in-class material
  from courses of the statistics and economics departments at the
  University of California in Berkeley and University of British
  Columbia but also influenced by others \cite{Steger01,Steger02}.}
\end{multicols*}
...
\begin{itemize}
\item Standard normal : $z \sim (0,1) ; \mu = 0 , \sigma = 1$
\item Chi-squared : $\chi^2_n = \sum^n z_i^2, \mu = n, \sigma = 2n$
\item t : let $x\sim\chi^2_n, Y = z/\sqrt{x/n} \rightarrow y \sim t_n$
\item F : let $x_1\sim \chi^2_{n1}$, and $x_2\sim \chi^2_{n2}$; $y = \frac{x_1/n_1}{x_2/n_2} \rightarrow Y \sim F{n_1,n_2}$
...
\begin{itemize}
\item Linearity : $Y = X\beta$
\item Strict Exogeneity : $E(\epsilon_i | X) = 0$
  \begin{itemize}
      \item Unconditional mean of error $E(\epsilon_i) = 0$
      \item Cross moment of residuals and regressors is zero, X is orthogonal to $\epsilon$ : $E(X_i \epsilon_i) = 0$
  \end{itemize}
\item No multicollinearity - $rank(X) = k$
\item Spherical error variance : $E(\epsilon_i^2|X) = \sigma^2 ; E(\epsilon \epsilon' |X) = \sigma^2 I_n$
\item $\epsilon|X \sim N(0,\sigma^2 I_n)$
\item ${(Y_i,X_i): i = 1, ..., n}$ are i.i.d.
\end{itemize}
...
\textbf{Generalised least squares}

If covariance matrix of errors is known: $E(\epsilon \epsilon' |X) = \Omega$
$$
\hat{\beta}_{GLS} = (X'\Omega^{-1}X)^{-1}X'\Omega^{-1}Y
$$
$$
\mathbb{V}(\hat{\beta}_{GLS}) = (X'\Omega^{-1}X)^{-1}
$$
\textbf{Restrited OLS}- optimise: $L(b,\lambda) = (Y-Xb)'(Y-Xb)+2\lambda(Rb-r)$

\textbf{Huber-White Sandwich 'Robust' SEs}
...
$$
which simplifies to $V(\beta) = \sigma^2(X'X)^{-1}$ because of the assumption $E(\epsilon \epsilon') = \sigma^2 I$.

If this is not true (i.e. heteroskedasticity is presentthe
variance covariance formula is
$$
E[(\hat{\beta} - \beta)(\hat{\beta} - \beta)'] =
(X'X)^{-1}X'ee'X(X'X)^{-1}
= \sigma_\epsilon^2 (X'X)^{-1}X' \Omega X(X'X)^{-1}
$$

$V(\hat{\beta}) =  Q^{-1}\Omega Q^{-1}$ Where,
$Q=\mathbb{E}X_iXi', \Omega = \mathbb{E}\hat{u_i}^2 X_i X_i'$
...
\begin{itemize}
\item $P_x = X(X'X)^{-1}X'$ - Hat Matrix - projector into $span(X)$
\item $M_x = I_n - P_x = I_n - X(X'X)^{-1}X'$ - Annihilator Matrix - projector into $span^{\bot}(X)$
\end{itemize}

\textbf{Fitted Value}: $\hat{Y} = P_x Y$

\textbf{Residual}: $e = M_x Y$


\subsection{Instrumental Variables}
Exogeneity violated when $E(X_i \epsilon_i) \neq 0$. OLS estimates no longer consistent.

\textbf{IV requirements:}
\begin{itemize}
  \item $Cov(Z,X) \neq 0$ - Relevance
  \item $Cov(Z,\epsilon) = 0 ; Z \bot \epsilon$ - Exogeneity / Exclusion restriction
  \item Affects Y only through X
  \item $dim(Z_i) \geq dim(X_i)$
\end{itemize}

\begin{itemize}
  \item \textbf{First Stage} : Regress X on Z
  \item \textbf{Reduced form} : Regress Y on Z
\end{itemize}


$$
\hat{\beta}_{iv} = (Z'X)^{-1}Z'Y =
\frac{Cov(Z,Y)}{Cov(Z,X)} = \frac{\hat{\beta}_{reduced form}}{\hat{\beta}_{first stage}}
$$

$$
\mathbb{V}(\hat{\beta}_{iv}) = Q_{zx}^{-1} \Omega Q_{xz}^{-1} ; \enspace \Omega = \mathbb{E}z_i z_i' u_i^2
$$
If $dim(Z_i) > dim(X_i)$ (more instruments than endogenous regressors),
$$
\hat{\beta}_{2SLS} =(X'P_zX)^{-1}X'P_zY =  (X'Z(Z'Z)^{-1}Z'X)^{-1}X'Z(Z'Z)^{-1}Z'Y
$$
$$
\mathbb{V}(\hat{\beta}_{2sls}) =
(Q_{xz} Q_{zz}^{-1} Q_{zx})^{-1} Q_{xz} Q_{zz}^{-1}
\Omega Q_{zz}^{-1} Q_{zx} (Q_{xz} Q_{zz}^{-1} Q_{zx})^{-1}  ;
\enspace \Omega = \mathbb{E}z_i z_i' u_i^2
$$
\subsection{Nonlinear Regressions}

Data:
$\{w_i: i = 1, \cdots , n \} \enspace iid$,
$w_i \sim f(w,\theta) , \theta \in \Theta \in \mathbb{R}_k$
...
$$
L_n(\theta) = \prod_{i=1}^n f(w_i,\theta)
$$

$$
log L_n(\theta) = \frac{1}{n} \sum_{i=1}^n log f(w_i,\theta)
$$

\textbf{MLE Estimator}

$$
\hat{\theta} = \text{arg max}_{\theta \in \Theta}
\frac{1}{n} \sum_{i=1}^{n}log f(w_i,\theta)
$$
...
$$
\Omega = \mathbb{E}
\frac{\partial log f(w_i,\theta_0)}{\partial \theta}
\frac{\partial log f(w_i,\theta_0)}{\partial \theta'}
= -B = I(\theta_0)
$$
...
\rightarrow_d N(0,B^{-1} \Omega B^{-1})
= N(0,[ I(\theta_0)]^{-1})
$$
...
$$
\mathbb{P}(Y_i = 1 | X_i) = 1 - \mathbb{F}(-X_i'\beta)
$$
...
$$
log L_n(\beta) = \frac{1}{n} \sum Y_i log \mathbb{F}(X_i'\beta)
\frac{1}{n} \sum(1-Y_i) log (1-\mathbb{F}(X_i'\beta))
$$

\textbf{Solutions for Logit} :
...
$$\mathbb{E}(Y_i|X_i) = \mathbb{P}(Y_i=1|X_i) = \Lambda(X_i'\beta)$$

\textbf{Variance}

$$
I(\beta) = \mathbb{E}[(\Lambda(X_i'\beta)(1-\Lambda(X_i'\beta)) X_i X_i' ]
$$

\textbf{Marginal Effects}

$$
\frac{\partial P(Y_i =1 | X_i)}{\partial X_i} = f(X_i'\beta) \beta
=\Lambda(X_i'\beta)(1-\Lambda(X_i'\beta)) \beta
$$
